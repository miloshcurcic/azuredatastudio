...
export default class MainController implements vscode.Disposable {
	...
	public async createNewProject(): Promise<void> {
		try {
			...
			if (!selectionResult) {
				vscode.window.showErrorMessage('Location is required to create a new databbase project.');
				return;
			}

			if (selectionResult.length > 0) {
				throw new Error(`Only one folder should be selected, but ${selectionResult.length} were.`);
			}
